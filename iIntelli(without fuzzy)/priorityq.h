...
void priorityq::add(string sen, double newweight)
{
    node n;
    n.sentence=sen;
    n.weight=newweight;
...
void priorityq::heapify(int index)
{
	...
	if(heap[index*2+1].weight<heap[index].weight && heap[index*2+2].weight<heap[index].weight){
	    flag=1;
	}else if(heap[index*2+1].weight>heap[index].weight && heap[index*2+1].weight>heap[index*2+2].weight){
	    flag=2;
	}else if(heap[index*2+2].weight>heap[index].weight && heap[index*2+2].weight>heap[index].weight){
	    flag=3;
	}
    }else if((index*2+1)<heap.size()){
	if(heap[index*2+1].weight<heap[index].weight){
	    flag=1;
	}else{
	    flag=2;
	}
    }else if((index*2+2)<heap.size()){
	if(heap[index*2+2].weight>heap[index].weight){
	    flag=3;
	}else{
	    flag=1;
	}
